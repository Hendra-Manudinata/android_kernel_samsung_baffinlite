...
#if defined(CONFIG_USB_SWITCH_FSA9485) || defined(CONFIG_RT8973)
...
#include <linux/platform_data/fsa9485.h>
#endif
...
#define SPA_CHECK_COUNT	500
#define SPA_CHECK_DELAY_TIME	20
...
static struct v4l2_subdev_sensor_interface_parms camdrv_ss_sub_if_params = {
	...
	.parms.serial = {
		...
		.phy_rate = 0,
		.pix_clk = 0,
		.hs_term_time = 0x4,     // Need to adjust for a sensor : Data lane termination time
		.hs_settle_time = 0x0    // Need to adjust for a sensor : Data lane termination time
	},
};
...
#if defined  (CONFIG_SENSORS_HSCDTD006A) || defined(CONFIG_SENSORS_HSCDTD008A) 
#ifdef CONFIG_MACH_BAFFINLITE_DTV_LTN
...
#define	MUIC_I2C_BUS_ID	8

#ifdef CONFIG_HAS_WAKELOCK
static struct wake_lock muic_jig_wakelock;
#endif
...
static void jig_wakelock_init(void)
{
#ifdef CONFIG_HAS_WAKELOCK
	wake_lock_init(&muic_jig_wakelock, WAKE_LOCK_SUSPEND,
		"muic_jig_wakelock");
#endif
...
{
#ifdef CONFIG_HAS_WAKELOCK
	if (wake_lock_active(&muic_jig_wakelock)) {
		wake_unlock(&muic_jig_wakelock);
...
static void usb_attach(uint8_t attached)
{
	...
	case CABLE_TYPE_USB:
		...
		//remove with pmic usb part charging. it operate in conflict with below part
#if defined(CONFIG_SEC_CHARGING_FEATURE)		
...
#if defined(CONFIG_SEC_CHARGING_FEATURE)
	for(i=0;i<SPA_CHECK_COUNT;i++){
		/* check spa is ready or not */
		if(spa_event_handler(SPA_EVT_CHARGER, spa_data) >= 0) break;

		msleep(SPA_CHECK_DELAY_TIME);
	}
#endif
...
static void charger_attach(uint8_t attached)
{
	...
	if((check_connected && attached) || (check_connected == 0 && attached == 0)) {
		pr_info("tsu6111_charger_cb double called return %d\n", attached);	
		return;
	}
	...
	for(i=0;i<SPA_CHECK_COUNT;i++){
		...
		printk("charger_attach before check count %d \n", i);
		/* check spa is ready or not */
		if(spa_event_handler(SPA_EVT_CHARGER, spa_data) >= 0) break;

		printk("charger_attach after check count %d \n", i);
...
static void fsa9485_audiodock_cb(int attached)
{
	...
	switch_set_state(&switch_dock, attached);
//ENABLE_HOST
	send_otg_insert_event(attached);
}
#endif
...
static struct fsa9485_platform_data fsa9485_pdata = {
	...
	.usb_cdp_cb = usb_attach, /*fix:Process USB when it is connected as a CDP type*/
...
static void __init hawaii_add_i2c_devices(void)
{
	...
	i2c_register_board_info(MUIC_I2C_BUS_ID, micro_usb_i2c_devices_info,
...
#if defined (CONFIG_LCD_SC7798_I9060_SUPPORT) || defined(CONFIG_LCD_HX8369_DTC_I9060_SUPPORT)
...
struct kona_fb_platform_data konafb_devices[] __initdata = {
	{
		...
		.reg_name = "cam2",
		.rst =  {
#if defined(CONFIG_MACH_JAVA_SS_BAFFINLITE_REV02)
			.gpio = 0,
#else
			.gpio = 22,
#endif			
...
#if defined (CONFIG_LCD_HX8369_SUPPORT) || defined (CONFIG_LCD_HX8369_BOE_I9060_SUPPORT) 
struct kona_fb_platform_data konafb_devices[] __initdata = {
	{
		.name = "HX8369",
		.reg_name = "cam2",
...
#if defined (CONFIG_LCD_HX8369_SUPPORT) || defined (CONFIG_LCD_HX8369_BOE_I9060_SUPPORT) || defined (CONFIG_LCD_HX8369_DTC_I9060_SUPPORT)
